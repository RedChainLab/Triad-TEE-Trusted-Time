...
typedef struct ms_main_thread_t {
	int ms_sleep_time;
	int ms_core_id;
	int ms_set_aff;
	int ms_sleep_inside_enclave;
} ms_main_thread_t;
...
typedef struct ms_readTSC_t {
	long long* ms_timestamps;
} ms_readTSC_t;

typedef struct ms_set_affinity_t {
	int ms_core_id;
} ms_set_affinity_t;
...
static sgx_status_t SGX_CDECL sgx_main_thread(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_main_thread_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_main_thread_t* ms = SGX_CAST(ms_main_thread_t*, pms);
	ms_main_thread_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_main_thread_t), ms, sizeof(ms_main_thread_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;


	main_thread(__in_ms.ms_sleep_time, __in_ms.ms_core_id, __in_ms.ms_set_aff, __in_ms.ms_sleep_inside_enclave);


	return status;
}

static sgx_status_t SGX_CDECL sgx_countADD(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_main_thread, 0, 1},
		{(void*)(uintptr_t)sgx_countADD, 0, 1},
...
sgx_status_t SGX_CDECL readTSC(long long* timestamps)
{
	...
	ms_readTSC_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_readTSC_t);
	void *__tmp = NULL;
	...
	ms = (ms_readTSC_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_readTSC_t));
	ocalloc_size -= sizeof(ms_readTSC_t);
...
sgx_status_t SGX_CDECL set_affinity(int core_id)
{
	...
	ms_set_affinity_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_set_affinity_t);
	void *__tmp = NULL;
	...
	ms = (ms_set_affinity_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_set_affinity_t));
	ocalloc_size -= sizeof(ms_set_affinity_t);

	if (memcpy_verw_s(&ms->ms_core_id, sizeof(ms->ms_core_id), &core_id, sizeof(core_id))) {
		sgx_ocfree();
		return SGX_ERROR_UNEXPECTED;
	}
...
sgx_status_t SGX_CDECL ocall_sleep(int* time)
{
	...
	if (time != NULL) {
		...
		if (memcpy_verw_s(__tmp, ocalloc_size, time, _len_time)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
...
sgx_status_t SGX_CDECL see_pid(const char* str)
{
	...
	ms_see_pid_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_see_pid_t);
	void *__tmp = NULL;
	...
	ms = (ms_see_pid_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_see_pid_t));
	ocalloc_size -= sizeof(ms_see_pid_t);
