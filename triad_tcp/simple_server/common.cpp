...
int receive_udp_packet(node_connection& nc, int& sockfd, unsigned char* key, unsigned char* iv, int* dest, int* misc, int* waiting_time) {
    ...
    if (received_bytes < 0) {
        ...
        ret = parseMessage((const char*) decryptedtext, fields, &nb_fields);//already_sent, &temp_ts, &type);
        if(ret == -1){
            ...
            t_print("COMMON : misc : %d\n", *misc);
            dest[0] = fields[0];
            *misc = fields[1];
            *waiting_time = fields[2];
            return CALIBRATION;
        }
        else if(ret == PORT){
            t_print("COMMON : bootrstap recquired\n");
...
int parseMessage(const char* message, int* fields, int* nb_fields) {
    ...
    int count = 1;
    if( message[0] == 'p' ){//format : "p;<dest_port_1>;<dest_port_2>;<dest_port_3>"
