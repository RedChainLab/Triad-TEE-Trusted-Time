...
Server::Server(int p, sync_struct* s): port(p), sync(s) {
...
int Server::handle_client(node_connection &client, int i){
    ...
    if(client.is_connected == 1){
        ...
        sync->misc = misc;
        if(res == CALIBRATION){
            sync->is_calibrating = 1;
            t_print("SERVER : calibration\n");
        }   
        for(int i = 0; i < NB_NODE; i++){
            sync->destination[i] = dest[i];
        }
        sync->mtx.unlock();
    }
    else{
